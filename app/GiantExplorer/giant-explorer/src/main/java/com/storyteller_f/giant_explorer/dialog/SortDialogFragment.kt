...
val activeSortChains = MutableLiveData<List<SortChain<FileSystemModel>>>()
...
class SortDialogFragment : CommonDialogFragment() {
    ...
    private val listener =
        object : DefaultDialog.Listener<SortChain<FileSystemModel>, SortConfigItem> {
            override fun onSaveState(oList: List<SortChain<FileSystemModel>>) =
                oList.map {
                    NameSort.Item()
                }.toMutableList()

            override fun onRestoreState(configItems: List<SortConfigItem>): List<SortChain<FileSystemModel>> =
                configItems.buildSorts()

            override fun onActiveChanged(activeList: List<SortChain<FileSystemModel>>) {
                activeSortChains.value = activeList
            }

            override fun onEditingChanged(editing: List<SortChain<FileSystemModel>>) {

            }
        }

    private val sortChains: List<SortChain<FileSystemModel>>
        get() = buildList {
