...
val activeFilters = MutableLiveData<List<Filter<FileSystemModel>>>()
...
class FilterDialogFragment : CommonDialogFragment() {
    ...
    private val listener =
        object : DefaultDialog.Listener<Filter<FileSystemModel>, FilterConfigItem> {
            override fun onSaveState(oList: List<Filter<FileSystemModel>>): List<FilterConfigItem> =
                oList.map {
                    (it as NameFilter).item
                }.toMutableList()

            override fun onActiveChanged(activeList: List<Filter<FileSystemModel>>) {
                activeFilters.value = activeList
            }

            override fun onEditingChanged(editing: List<Filter<FileSystemModel>>) {
            }

            override fun onRestoreState(configItems: List<FilterConfigItem>): List<Filter<FileSystemModel>> {
                return configItems.buildFilters()
            }
        }

    private val filters: List<Filter<FileSystemModel>>
        get() = buildList {
